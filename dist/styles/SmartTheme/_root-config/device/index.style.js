...
const breakpoints_style_1 = require("./breakpoints.style");
const gerarClassesCss = function () {
    let str = "";
    let strMaxSizeContent = "";
    for (let key in breakpoints_style_1.breakpoints) {
        let bkp = breakpoints_style_1.breakpoints[key];
      ...
      --screen-max-size-content: calc( calc( var(--screen-breakpoint) * 1rem) - calc( var(--screen-sangria) * 2rem) );`;
            ...
            strMaxSizeContent = `
      --screen-max-size-content: calc( calc( var(--${breakpoints_style_1.maxContentWidthFromBreakpoint}-breakpoint) * 1rem) - calc( var(--screen-sangria) * 2rem) );`;
        }
        str = str +
            `
      :root {
